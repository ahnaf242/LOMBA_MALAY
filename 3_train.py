...
from monai.transforms import (
    ...
    SpatialPadd # <-- PENYELAMAT CRASH KITA
)
...
def main():
    print(f"🚀 Memulai persiapan di device: {DEVICE}")
    ...
    if not data_dicts:
        print("Data tidak ditemukan. Pastikan path folder sudah benar!")
        return

    # Split: 80% untuk belajar, 20% untuk ujian harian
    ...
    train_transforms = Compose([
        ...
        # Mengambil potongan gambar 96x96x64, dipaksa 3x lebih sering melihat jantung
        RandCropByPosNegLabeld(
            keys=["image", "label"],
            label_key="label",
            spatial_size=(96, 96, 64),
            pos=3, neg=1, num_samples=2,
        ),
    ...
    for epoch in range(EPOCHS):
        ...
        with torch.no_grad():
            ...
            if metric > best_dice:
                best_dice = metric
                torch.save(model.state_dict(), "best_3d_resunet_pro.pth")
                print(f"REKOR BARU! Model disimpan dengan Dice: {best_dice:.4f}")

    print(f"\nTRAINING SELESAI!")
    print(f"Skor Dice tertinggi: {best_dice:.4f}")
